...
class Counters:
    def __init__(self):
        ...
        self.client_wildcard_counts = defaultdict(int)  # New: Count wildcard probes per client
                ...
                for elt in pkt[Dot11Elt]:
                    if elt.ID == 0:  # SSID element
                        ...
                        if not ssid:  # Empty SSID = wildcard/broadcast probe
                            ...
                            counters.client_wildcard_counts[client_mac] += 1
                        else:
...
# New Table: Wildcard probe requests per client
print("\nAnalysis: Wildcard probe requests per client:")
print("| Client MAC | Vendor | Wildcard Count |")
print("|------------|--------|----------------|")
for client in sorted(counters.unique_clients):
    redacted_client = redact_mac(client)
    vendor = get_vendor(client)
    count = counters.client_wildcard_counts.get(client, 0)
    print(f"| {redacted_client} | {vendor} | {count} |")

# New Table: Count of vendors by unique MAC (descending)
vendor_counts = defaultdict(int)
for mac in counters.unique_clients:
    vendor = get_vendor(mac)
    vendor_counts[vendor] += 1

print("\nCount of unique MACs per vendor (descending):")
print("| Vendor | Count |")
print("|--------|-------|")
